...
def get_user_inputs(date):
    ...
    if option == str(1) :
        ...
        response_all_states = requests.get(url=GET_STATES_ENDPOINT, headers=headers)
        ...
        request_url_state = f"{FIND_BY_STATE_ENDPOINT}{state_id}"

        response_state = requests.get(url = request_url_state, headers=headers)
...
def get_info_for_pin_code(pin_code,date):
    ...
    request_url_pin = f"{FIND_BY_PIN_ENDPOINT}{pin_code}&date={date}"

    # Get request for slot availability based on pin
    response_pin = requests.get(url = request_url_pin, headers=headers)
...
def fetch_data_district_id(district_id,child_conn) :
    ...
    request_url_district = f"{FIND_BY_DISTRICT_ENDPOINT}{district_id}&date={date}"
    # get response for district
    response_district = requests.get(url=request_url_district, headers=headers)
